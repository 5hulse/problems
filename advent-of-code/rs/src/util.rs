...
pub fn load_input(year: u32, day: u32) -> Result<String, Box<dyn Error>> {
    let mut file = File::open(&format!("../inputs/{}-{:02}/main.txt", year, day))?;
    let mut input = String::new();
    file.read_to_string(&mut input)?;
    Ok(input)
}

pub fn load_example(year: u32, day: u32, part: u32) -> Result<(String, i32), Box<dyn Error>> {
    let mut file = File::open(&format!(
        "../inputs/{}-{:02}/part{}-example.toml",
        year, day, part
    ))?;
    ...
    let input: String = match table.get("input") {
        Some(Value::String(s)) => Ok(s.to_owned()),
        Some(_) => Err("input field isn't a String!"),
        None => Err("couldn't read input field"),
    }?;
    let output = match table.get("output") {
        Some(Value::Integer(s)) => Ok(*s),
        Some(_) => Err("output field isn't an Integer!"),
        None => Err("couldn't read output field"),
    }? as i32;
...
pub mod tests {
    ...
    #[test]
    pub fn test_load_example() {
        let (input, output) =
            load_example(2023, 1, 1).expect("`load_example` encountered an error");
        assert_eq!(
            input,
            String::from("1abc2\npqr3stu8vwx\na1b2c3d4e5f\ntreb7uchet\n")
        );
    ...
    #[test]
    pub fn test_load_input() {
        let input = load_input(2023, 1).expect("`load_input` encountered an error");
        assert_eq!(
            input,
            r#"rhqrpdxsqhgxzknr2foursnrcfthree
2bmckl
four95qvkvveight5
